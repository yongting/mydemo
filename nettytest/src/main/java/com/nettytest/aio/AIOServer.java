...
public class AIOServer {
	...
	public AIOServer() throws IOException{
        ...
        //绑定端口并设置连接请求队列长度
        server.bind(new InetSocketAddress(8000), 1024);    
    }
					...
					public void completed(Integer count, Object attachment) {
						...
						if (count > 0) {
							try {
								...
								/*
								 * //写入也是异步调用，
								 * 也可以使用传入CompletionHandler对象的方式来处理写入结果
								 * //channel.write(CharsetHelper.encode(
								 * CharBuffer.wrap(answer))); try{
								 * channel.write(Charset.forName("UTF-8").
								 * newEncoder().encode(CharBuffer.wrap(answer)))
								 * ; } //Unchecked exception thrown when an
								 * attempt is made to write to an asynchronous
								 * socket channel and a previous write has not
								 * completed. //看来操作系统也不可靠
								 * catch(WritePendingException wpe){
								 * //休息一秒再重试，如果失败就不管了 Helper.sleep(1);
								 * channel.write(Charset.forName("UTF-8").
								 * newEncoder().encode(CharBuffer.wrap(answer)))
								 * ; }
								 */
